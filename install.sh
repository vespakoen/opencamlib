...
verify_args() {
    if [ -n "${OCL_CLEAN}" ] && [ -z "${OCL_BUILD_LIBRARY}" ]; then
        ...
        echo "Cannot set --install-prefix without setting --install or --sudo-install. add --install or --sudo-install option or remove the --install-prefix option"
        exit 1
    elif [ -n "${OCL_BOOST_WITH_PYTHON}" ] && [ -z "${OCL_INSTALL_BOOST}" ]; then
        echo "Cannot set --boost-with-python without setting --install-boost. add --install-boost or remove the --boost-with-python option"
        exit 1
    elif [ -n "${OCL_BOOST_ADDRESS_MODEL}" ] && [ -z "${OCL_INSTALL_BOOST}" ]; then
        echo "Cannot set --boost-address-model without setting --install-boost. add --install-boost or remove the --boost-address-model option"
        exit 1
    elif [ -n "${OCL_BOOST_ARCHITECTURE}" ] && [ -z "${OCL_INSTALL_BOOST}" ]; then
        echo "Cannot set --boost-architecture without setting --install-boost. add --install-boost or remove the --boost-address-model option"
        exit 1
    elif [ -n "${OCL_BOOST_PYTHON_VERSION}" ] && [ -z "${OCL_INSTALL_BOOST}" ]; then
        echo "Cannot set --boost-python-version without setting --install-boost. add --install-boost or remove the --boost-python-version option"
        exit 1
    fi
}
verify_args
...
install_boost () {
    cd "${project_dir}"
    if [ -f boost-precompiled.tar.gz ]; then
...
if [ -n "${OCL_DOCKER_IMAGE}" ]; then
    ...
    read -a ALL_OPTIONS <<< "$(compgen -A variable | grep '^OCL_')"
